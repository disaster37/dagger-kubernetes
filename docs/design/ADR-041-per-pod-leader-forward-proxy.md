...
### 2. Internal forward hop: HTTPS with full certificate verification

The hop targets the leader's **control port** (`server.control_addr`, 8080)
over the leader's **pod FQDN** (`LeaderAddress()` host). Trust is per provider:

- **Embedded (default):** each pod's server certificate gets its **own exact,
  deterministic pod-FQDN SANs** — `<hostname>.<headless>.<ns>.svc.<clusterDomain>`
  and the `.svc`-short form `<hostname>.<headless>.<ns>.svc` (for the
  `raft.cluster_domain=""` override), where `hostname` is the pod name from
  `os.Hostname()` and the remaining values come from the existing Raft
  discovery config (`repository.ServerCertFQDNSANs`). No wildcard and no
  all-pod enumeration: the only host a follower ever dials is the leader's own
  FQDN, so per-pod exact SANs are complete for verification and strictly
  stronger than a wildcard (one key never covers other names), and new pods
  issue their own certs at boot with zero cross-pod coupling. Followers verify
  with `RootCAs` = the shared minting-CA pool (`ca.minting_ca_secret`), exactly
  as the Dagger CLI verifies the embedded data-plane cert.
- **cert-manager / external:** the server certificate is operator-managed, so
  the operator must add the wildcard SAN
  `*.<headless>.<ns>.svc.<clusterDomain>` (+ the `.svc` form) to their
  `Certificate`/Secret so follower→leader hops verify. ACME wildcards require a
  DNS-01 solver; a private CA adds the SAN trivially. Followers use the system
  pool (nil `RootCAs`), or a mounted private-CA bundle added to the pod trust
  store.
...
embedded `server.crt`/`server.key` that predate the own-FQDN SANs are
transparently re-issued in place under the same CA on the first boot after the
upgrade (`EmbeddedProvider.ServerTLSCert` re-checks SAN coverage via
`coversRequiredServerSANs`), mirroring the raft-leaf re-issue behavior
(ADR-029).
...
- One internal HTTPS hop (with full verification) and one internal L4 relay are
  added. The relay-in port must stay pod-to-pod only.

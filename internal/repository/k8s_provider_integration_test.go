...
func TestRealK8sEnsureStatefulSetCreate(t *testing.T) {
	...
	if sts.Name != domain.StsName(version) {
		t.Errorf("expected name %q, got %q", domain.StsName(version), sts.Name)
	}

	if sts.Spec.ServiceName != domain.ServiceName(version) {
		t.Errorf("expected serviceName %q, got %q", domain.ServiceName(version), sts.Spec.ServiceName)
	}
...
func TestRealK8sProviderIntegration(t *testing.T) {
	...
	sessions := service.NewStore(5 * time.Minute)
	manager := service.NewManager(p, sessions, service.ManagerConfig{
	...
	resultChan := make(chan *domain.AcquireResult, 1)
	...
	t.Logf("Integration flow: STS=%s replicas=%d ready, %d pods found",
		domain.StsName(version), *sts.Spec.Replicas, len(replicas))
}

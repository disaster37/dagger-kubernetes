...
| `helm`   | Lint delegated to `helm` module; template matrix local | Upstream `Lint` = `helm dependency update` + `helm lint`; no `helm template` support. The local template matrix also covers the three data-plane TLS cases (embedded default, `dataCert.enabled`, `dataIngress.tls.secretName`) plus the external-provider keypair rendering, a `minio.enabled=false` variant that renders the chart without the bundled S3 object store, a subchart-`fullnameOverride` variant that renames tempo/loki/victoria/minio/opentelemetry-collector (in lock step with the `global.daggerKubernetes.serviceNames.*` collector-exporter keys), and a `dex.enabled=true` + `openldap.enabled=true` variant that renders the optional Dex/OpenLDAP OIDC subcharts (both disabled by default; the default variant proves they stay out when disabled). The default and subchart-rename variants additionally assert URL substrings in the rendered manifests — the default variant covers both the supervisor ConfigMap URLs and the collector's own exporter endpoints under their default Service names, the rename variant the renamed ones — so the matrix proves the auto-wired service URLs follow each dependency's own fullname rules instead of only checking that rendering succeeds; the dex/openldap variant asserts the `dagger-kubernetes-dex` Deployment name appears. |
